...
@Tag(name = "Связи артефактов и атрибутов", description = "Контроллер по настройке связи артефактов и атрибутов")
public class  ArtifactsAttributesController {
    ...
    public ResponseEntity<ArtifactsAttributesDto> create(
        ...
        return ResponseEntity.status(201).body(artifactsAttributesService.create(artifactsAttributesCreateDto));
    }

    @Operation(summary = "Обновление cвязи артефактов и атрибутов", description = "Обновляет cвязи артефактов и атрибутов")
    @PutMapping
