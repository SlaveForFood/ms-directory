...
public class HeroesSkillsServiceImpl implements HeroesSkillsService {
  ...
  public HeroesSkillsDto create(HeroesSkillsCreateDto heroesSkillsCreateDto) {
    HeroesSkills heroesSkills =
        heroesSkillsRepo.save(
            heroesSkillsMapper.heroesSkillsCreateDtoToHeroesSkills(heroesSkillsCreateDto));
  ...
  public List<HeroesSkillsDto> getAll() {
    return heroesSkillsRepo.findAll().stream()
        .map(heroesSkillsMapper::heroesSkillToHeroesSkillsDto)
        .toList();
  }

  @Override
  public HeroesSkillsDto getById(Integer id) {
    return heroesSkillsMapper.heroesSkillToHeroesSkillsDto(
        heroesSkillsRepo
            .findById(id)
            .orElseThrow(
                () ->
                    new NotFoundException(
                        String.format("Связь героя и навыка с id=%d не найден", id))));
  }
